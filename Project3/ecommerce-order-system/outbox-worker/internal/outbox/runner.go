...
import (
	...
	"ecommerce-order-system/outbox-worker/metrics"
...
type Runner struct {
	...
	EventsPub *rabbit.Publisher // orders.events
...
func (r *Runner) tick(ctx context.Context) error {
	...
	// обновим pending gauge периодически (даже если batch пуст)
	if err := r.updatePending(ctx); err == nil {
		// ok
	}
	...
	for _, e := range batch {
		if e.Attempts >= r.MaxAttempts {
			_, _ = tx.Exec(ctx, `
				...
				set last_error=$2, sent_at=now()
				where id=$1
			`, e.ID, fmt.Sprintf("max attempts reached: %d", e.Attempts))
